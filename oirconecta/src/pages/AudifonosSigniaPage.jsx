...
import { ArrowForward, CheckCircle, Bluetooth, BatteryChargingFull, Smartphone, Hearing, RecordVoiceOver } from '@mui/icons-material';

const BRAND = {
  nombre: 'Signia',
  logo: '/logos/marcas/Signia-logo.png',
  eslogan: 'Own Voice Processing™',
  descripcion: 'Signia combina innovación, diseño y conectividad para que vivas cada momento con confianza y estilo. Tecnología Augmented Xperience y soluciones invisibles para cada necesidad auditiva.',
  rating: '4.8',
  gradient: 'linear-gradient(135deg, #DC143C 0%, #272F50 100%)',
  glow: 'rgba(220,20,60,0.40)',
};

const productos = [
  { nombre: 'Styletto AX', categoria: 'RIC', descripcion: 'Diseño ultradelgado, recargable y con tecnología Augmented Xperience', caracteristicas: ['Diseño ultradelgado y elegante', 'Recargable y portátil', 'Conectividad Bluetooth', 'Tecnología Augmented Xperience'], destacado: true },
  { nombre: 'Pure Charge&Go AX', categoria: 'BTE', descripcion: 'Recargable con sonido natural y conectividad avanzada', caracteristicas: ['Carga rápida y autonomía extendida', 'Sonido natural y claro', 'App Signia para control total', 'Conectividad avanzada'], destacado: false },
  { nombre: 'Silk X', categoria: 'ITE', descripcion: 'Invisible, cómodo y listo para usar con tecnología Xperience', caracteristicas: ['Invisible y cómodo', 'Listo para usar sin moldes', 'Sonido personalizado', 'Tecnología Signia Xperience'], destacado: false },
];

const tecnologias = [
  { icon: RecordVoiceOver, titulo: 'Own Voice Processing™', descripcion: 'Tu propia voz suena completamente natural', gradient: 'linear-gradient(135deg, #DC143C 0%, #272F50 100%)' },
  ...
  { icon: BatteryChargingFull, titulo: 'Recargable', descripcion: 'Carga rápida con autonomía extendida todo el día', gradient: 'linear-gradient(135deg, #DC143C 0%, #272F50 100%)' },
  { icon: Smartphone, titulo: 'App Signia', descripcion: 'Control total y personalización desde tu móvil', gradient: 'linear-gradient(135deg, #DC143C 0%, #272F50 100%)' },
];

const AudifonosSigniaPage = () => {
  const navigate = useNavigate();
  return (
    <>
      <Helmet>
        <title>Audífonos Signia - OírConecta | Own Voice Processing™</title>
        <meta name="description" content="Descubre los audífonos Signia con Own Voice Processing™. Styletto AX, Pure Charge&Go AX y Silk X con diseño elegante y conectividad avanzada." />
        <link rel="canonical" href="https://oirconecta.com/audifonos/signia" />
      </Helmet>
      <Header />
      ...
      <Box sx={{
        ...
        background:
          'radial-gradient(ellipse 90% 70% at 10% 20%, rgba(220,20,60,0.42) 0%, transparent 55%),' +
          'radial-gradient(ellipse 70% 60% at 90% 80%, rgba(39,47,80,0.55) 0%, transparent 55%),' +
          'linear-gradient(160deg, #7a1a30 0%, #a01a3a 30%, #272F50 70%, #1a1f38 100%)',
          ...
          <Grid container spacing={6} alignItems="center">
            <Grid item xs={12} md={6}>
              ...
              <Box sx={{ display: 'inline-flex', alignItems: 'center', px: 2.5, py: 1.5,
                ...
                <img src={BRAND.logo} alt="Signia" style={{ height: 48, objectFit: 'contain' }} />
              </Box>
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                <Button variant="contained" size="large" endIcon={<ArrowForward />}
                  onClick={() => navigate('/contacto?asunto=Solicitud%20de%20información%20-%20Signia')}
                  sx={{ borderRadius: '14px', fontWeight: 800, px: 3.5, py: 1.75, fontSize: '1rem',
                    background: '#fff !important', color: '#272F50',
              ...
              <Box sx={{ display: 'flex', justifyContent: 'center' }}>
                <Box sx={{ borderRadius: '8px', overflow: 'hidden', width: '100%', maxWidth: 440,
                  ...
                  <Box sx={{ borderRadius: '8px', bgcolor: '#fff', p: 3, mb: 3, textAlign: 'center' }}>
                    <img src={BRAND.logo} alt="Signia" style={{ height: 80, objectFit: 'contain' }} />
                  </Box>
                  {/* Stats */}
                  <Grid container spacing={2}>
                    {[
                      ...
                      { value: 'AX', label: 'Tecnología' },
                      { value: 'BT', label: 'Conectividad' },
                    ].map((s) => (
            ...
            {productos.map((p, i) => (
              <Grid item xs={12} sm={6} md={4} key={i}>
                <Box sx={{ height: '100%', borderRadius: '22px', overflow: 'hidden', bgcolor: '#fff',
                  boxShadow: '0 2px 20px rgba(220,20,60,0.08)', border: '1px solid rgba(220,20,60,0.08)',
                  display: 'flex', flexDirection: 'column',
                  transition: 'all 0.28s ease',
                  '&:hover': { transform: 'translateY(-6px)', boxShadow: '0 20px 48px rgba(220,20,60,0.15)' },
                  position: 'relative',
                }}>
                  {p.destacado && (
                    <Box sx={{ position: 'absolute', top: 12, right: 12, zIndex: 2,
                      px: 1.5, py: 0.5, borderRadius: '8px',
                      background: 'linear-gradient(135deg, #272F50, #fca5a5)',
                  ...
                  <Box sx={{ p: 2.5, flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
                      <Chip label={p.categoria} size="small" sx={{ fontWeight: 700, fontSize: '0.7rem',
                        bgcolor: 'rgba(220,20,60,0.08)', color: '#272F50', border: '1px solid rgba(220,20,60,0.18)' }} />
                    </Box>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75, mb: 2.5 }}>
                      {p.caracteristicas.slice(0, 3).map((c) => (
                        <Box key={c} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <CheckCircle sx={{ fontSize: 15, color: '#272F50' }} />
                          <Typography sx={{ fontSize: '0.8125rem', color: '#4a5568' }}>{c}</Typography>
                        </Box>
                      ))}
                    </Box>
                    <Button variant="contained" fullWidth onClick={() => navigate('/contacto?asunto=Solicitud%20de%20información%20-%20Signia')}
                      sx={{ borderRadius: '12px', fontWeight: 700,
                        background: 'linear-gradient(135deg, #272F50, #fca5a5)',
                        boxShadow: '0 4px 14px rgba(220,20,60,0.25)',
                        '&:hover': { boxShadow: '0 6px 20px rgba(220,20,60,0.35)' } }}>
                      Solicitar información
                    </Button>
                  </Box>
                </Box>
              ...
              return (
                <Grid item xs={12} sm={6} md={3} key={t.titulo}>
                  <Box sx={{ borderRadius: '22px', p: 3, height: '100%', textAlign: 'center',
                    ...
                    border: '1px solid rgba(220,20,60,0.08)', boxShadow: '0 2px 16px rgba(220,20,60,0.07)',
                    transition: 'all 0.28s ease',
                    '&:hover': { transform: 'translateY(-6px)', boxShadow: '0 20px 48px rgba(220,20,60,0.14)' } }}>
                    <Box sx={{ width: 64, height: 64, borderRadius: '8px', background: t.gradient,
                      display: 'flex', alignItems: 'center', justifyContent: 'center', mx: 'auto', mb: 2,
                      boxShadow: '0 8px 20px rgba(220,20,60,0.25)' }}>
      ...
      <Box sx={{ position: 'relative', overflow: 'hidden', py: { xs: 5, md: 8 },
        background:
          'radial-gradient(ellipse 80% 60% at 20% 50%, rgba(220,20,60,0.55) 0%, transparent 55%),' +
          'radial-gradient(ellipse 60% 70% at 80% 40%, rgba(220,20,60,0.40) 0%, transparent 55%),' +
          'linear-gradient(160deg, #7a1a30 0%, #a01a3a 30%, #272F50 70%, #1a1f38 100%)',
        color: '#fff' }}>
        ...
        <Container maxWidth="md" sx={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
          ...
          <Typography sx={{ fontSize: '1.125rem', color: 'rgba(255,255,255,0.78)', mb: 5, maxWidth: 500, mx: 'auto', lineHeight: 1.65 }}>
            Un especialista de la red te orienta sobre los modelos Signia que mejor se adaptan a tu caso.
          </Typography>
...
export default AudifonosSigniaPage;
