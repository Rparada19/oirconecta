...
const BRAND = {
  nombre: 'Unitron',
  logo: '/logos/marcas/unitron-logo.svg',
  eslogan: 'Flex:trial™',
  descripcion: 'Audífonos con tecnología Flex:trial™, recargables y conectividad total para una experiencia auditiva personalizada. Blu, Discover Next y app Remote Plus para cada necesidad auditiva.',
  rating: '4.8',
  gradient: 'linear-gradient(135deg, #0066B2 0%, #272F50 100%)',
  glow: 'rgba(0,102,178,0.40)',
};

const productos = [
  { nombre: 'Unitron Blu', categoria: 'RIC', descripcion: 'Audífono con tecnología Flex:trial™, recargable y control total desde la app.', caracteristicas: ['Tecnología Flex:trial™', 'Bluetooth y recargable', 'App Remote Plus', 'Resistente al agua'], destacado: true },
  { nombre: 'Unitron Discover Next', categoria: 'BTE', descripcion: 'Sonido natural y claro con diseño elegante y discreto.', caracteristicas: ['Sonido natural y claro', 'Recargable', 'App Remote Plus', 'Diseño elegante y discreto'], destacado: false },
  { nombre: 'Unitron Moxi Fit', categoria: 'ITE', descripcion: 'Diseño personalizado, conectividad avanzada y sonido potente.', caracteristicas: ['Diseño personalizado', 'Conectividad avanzada', 'App Remote Plus', 'Sonido potente y natural'], destacado: false },
];

const tecnologias = [
  { icon: Hearing, titulo: 'Flex:trial™', descripcion: 'Tecnología única para probar antes de comprar tu audífono', gradient: 'linear-gradient(135deg, #0066B2 0%, #272F50 100%)' },
  { icon: Bluetooth, titulo: 'Conectividad Total', descripcion: 'Bluetooth y streaming directo a cualquier dispositivo', gradient: 'linear-gradient(135deg, #0066B2 0%, #272F50 100%)' },
  { icon: BatteryChargingFull, titulo: 'Recargable', descripcion: 'Batería recargable y resistente al agua IP68', gradient: 'linear-gradient(135deg, #0066B2 0%, #272F50 100%)' },
...
const AudifonosUnitronPage = () => {
  ...
  return (
    <>
      <Helmet>
        <title>Audífonos Unitron - OírConecta | Flex:trial™</title>
        <meta name="description" content="Descubre los audífonos Unitron con tecnología Flex:trial™. Blu, Discover Next y Moxi Fit con conectividad Bluetooth y app Remote Plus." />
        <link rel="canonical" href="https://oirconecta.com/audifonos/unitron" />
      </Helmet>
      <Header />
      ...
      <Box sx={{
        ...
        background:
          'radial-gradient(ellipse 90% 70% at 10% 20%, rgba(0,102,178,0.42) 0%, transparent 55%),' +
          'radial-gradient(ellipse 70% 60% at 90% 80%, rgba(39,47,80,0.55) 0%, transparent 55%),' +
          'linear-gradient(160deg, #001e58 0%, #00215a 30%, #272F50 70%, #1a1f38 100%)',
          ...
          <Grid container spacing={6} alignItems="center">
            <Grid item xs={12} md={6}>
              ...
              <Box sx={{ display: 'inline-flex', alignItems: 'center', px: 2.5, py: 1.5,
                ...
                <img src={BRAND.logo} alt="Unitron" style={{ height: 48, objectFit: 'contain' }} />
              </Box>
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                <Button variant="contained" size="large" endIcon={<ArrowForward />}
                  onClick={() => navigate('/contacto?asunto=Solicitud%20de%20información%20-%20Unitron')}
                  sx={{ borderRadius: '14px', fontWeight: 800, px: 3.5, py: 1.75, fontSize: '1rem',
                    bgcolor: '#fff', color: '#0066B2',
              ...
              <Box sx={{ display: 'flex', justifyContent: 'center' }}>
                <Box sx={{ borderRadius: '8px', overflow: 'hidden', width: '100%', maxWidth: 440,
                  ...
                  <Box sx={{ borderRadius: '8px', bgcolor: '#fff', p: 3, mb: 3, textAlign: 'center' }}>
                    <img src={BRAND.logo} alt="Unitron" style={{ height: 80, objectFit: 'contain' }} />
                  </Box>
                  {/* Stats */}
            ...
            {productos.map((p, i) => (
              <Grid item xs={12} sm={6} md={4} key={i}>
                <Box sx={{ height: '100%', borderRadius: '22px', overflow: 'hidden', bgcolor: '#fff',
                  boxShadow: '0 2px 20px rgba(0,102,178,0.08)', border: '1px solid rgba(0,102,178,0.08)',
                  display: 'flex', flexDirection: 'column',
                  transition: 'all 0.28s ease',
                  '&:hover': { transform: 'translateY(-6px)', boxShadow: '0 20px 48px rgba(0,102,178,0.15)' },
                  position: 'relative',
                }}>
                  {p.destacado && (
                    <Box sx={{ position: 'absolute', top: 12, right: 12, zIndex: 2,
                      px: 1.5, py: 0.5, borderRadius: '8px',
                      background: 'linear-gradient(135deg,#0066B2,#0066B2)',
                      boxShadow: '0 4px 12px rgba(0,102,178,0.35)' }}>
                  ...
                  <Box sx={{ p: 2.5, flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
                      <Chip label={p.categoria} size="small" sx={{ fontWeight: 700, fontSize: '0.7rem',
                        bgcolor: 'rgba(0,102,178,0.08)', color: '#0066B2', border: '1px solid rgba(0,102,178,0.18)' }} />
                    </Box>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75, mb: 2.5 }}>
                      {p.caracteristicas.slice(0, 3).map((c) => (
                        <Box key={c} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <CheckCircle sx={{ fontSize: 15, color: '#0066B2' }} />
                          <Typography sx={{ fontSize: '0.8125rem', color: '#4a5568' }}>{c}</Typography>
                        </Box>
                      ))}
                    </Box>
                    <Button variant="contained" fullWidth onClick={() => navigate('/contacto?asunto=Solicitud%20de%20información%20-%20Unitron')}
                      sx={{ borderRadius: '12px', fontWeight: 700,
                        background: 'linear-gradient(135deg,#0066B2,#0066B2)',
                        boxShadow: '0 4px 14px rgba(0,102,178,0.25)',
                        '&:hover': { boxShadow: '0 6px 20px rgba(0,102,178,0.35)' } }}>
                      Solicitar información
                    </Button>
                  </Box>
                </Box>
              ...
              return (
                <Grid item xs={12} sm={6} md={3} key={t.titulo}>
                  <Box sx={{ borderRadius: '22px', p: 3, height: '100%', textAlign: 'center',
                    ...
                    border: '1px solid rgba(0,102,178,0.08)', boxShadow: '0 2px 16px rgba(0,102,178,0.07)',
                    transition: 'all 0.28s ease',
                    '&:hover': { transform: 'translateY(-6px)', boxShadow: '0 20px 48px rgba(0,102,178,0.14)' } }}>
                    <Box sx={{ width: 64, height: 64, borderRadius: '8px', background: t.gradient,
                      display: 'flex', alignItems: 'center', justifyContent: 'center', mx: 'auto', mb: 2,
                      boxShadow: '0 8px 20px rgba(0,102,178,0.25)' }}>
      ...
      <Box sx={{ position: 'relative', overflow: 'hidden', py: { xs: 5, md: 8 },
        background:
          'radial-gradient(ellipse 80% 60% at 20% 50%, rgba(0,102,178,0.50) 0%, transparent 55%),' +
          'radial-gradient(ellipse 60% 70% at 80% 40%, rgba(0,102,178,0.40) 0%, transparent 55%),' +
          'linear-gradient(160deg, #001e58 0%, #00215a 30%, #272F50 70%, #1a1f38 100%)',
        color: '#fff' }}>
        ...
        <Container maxWidth="md" sx={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
          ...
          <Typography sx={{ fontSize: '1.125rem', color: 'rgba(255,255,255,0.78)', mb: 5, maxWidth: 500, mx: 'auto', lineHeight: 1.65 }}>
            Un especialista de la red te orienta sobre los modelos Unitron que mejor se adaptan a tu caso.
          </Typography>
...
export default AudifonosUnitronPage;
