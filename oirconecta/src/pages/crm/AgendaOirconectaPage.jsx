/**
 * Agenda matriz de OírConecta (centro propio) dentro del CRM.
 *
 * Es LA fuente única de la agenda: configura horario, tipos de consulta y
 * bloqueos que el motor de slots usa para /agendar, la ficha pública del
 * directorio y el bot de WhatsApp. Consume /api/crm/retail-agenda/* (opera el
 * DirectoryProfile del retail con el token del CRM).
 *
 * Tabs: Próximas citas · Calendario · Configuración · Tipos · Horario · Bloqueos.
 */
...
import {
  ...
  TableRow, TableCell, TableBody, Grid, Tooltip,
} from '@mui/material';
import {
  EventOutlined, AddOutlined, DeleteOutlineOutlined, SaveOutlined,
  EditOutlined, BlockOutlined, CalendarMonthOutlined, CalendarToday,
...
const ACCENT = '#15803d';
const NAVY = '#0F2A4A';
const BASE = '/api/crm/retail-agenda';
...
export default function AgendaOirconectaPage() {
  const { user } = useAuth();
  ...
  const loadAll = useCallback(async () => {
    setLoading(true);
    const cfg = await api.get(`${BASE}/config`);
    if (cfg.error) {
    ...
    const [t, a, b, ap] = await Promise.all([
      api.get(`${BASE}/types?includeInactive=1`),
      api.get(`${BASE}/availability`),
      api.get(`${BASE}/blocks`),
      api.get(`${BASE}/appointments`),
    ]);
  ...
  if (loadError) {
    return (
      <Box sx={{ p: { xs: 2, md: 3 } }}>
        <PageHeader icon={CalendarToday} title="Agenda OírConecta" subtitle="Horario, tipos de consulta y bloqueos del centro propio" />
        <Alert severity="error" sx={{ mt: 2, borderRadius: '10px' }}>{loadError}</Alert>
      </Box>
    );
  }
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 } }}>
      <PageHeader icon={CalendarToday} title="Agenda OírConecta"
        subtitle="Fuente única de la agenda. Lo que definas acá gobierna /agendar, la ficha del directorio y el bot de WhatsApp." />

      {!config?.agendaActiva && (
        <Alert severity="warning" sx={{ mt: 2, borderRadius: '10px' }}>
          La agenda en línea está <strong>pausada</strong>. Mientras siga así, no se podrán reservar citas desde el sitio ni WhatsApp. Actívala en la pestaña Configuración.
        </Alert>
      )}
      ...
      <Card sx={{ mt: 2, borderRadius: '14px', border: '1px solid #e5e7eb' }}>
        ...
        <Box sx={{ p: { xs: 2, md: 3 } }}>
          ...
          {tab === 2 && <ConfigTab config={config} setConfig={setConfig} showSnack={showSnack} isAdmin={isAdmin} />}
          {tab === 3 && <TypesTab types={types} setTypes={setTypes} showSnack={showSnack} isAdmin={isAdmin} />}
          {tab === 4 && <AvailabilityTab availability={availability} setAvailability={setAvailability} showSnack={showSnack} isAdmin={isAdmin} />}
...
function ConfigTab({ config, setConfig, showSnack, isAdmin }) {
  ...
  const save = async () => {
    setSaving(true);
    const r = await api.put(`${BASE}/config`, form);
    setSaving(false);
  ...
  return (
    <Box>
      {!isAdmin && <Alert severity="info" sx={{ mb: 2, borderRadius: '10px' }}>Solo un administrador puede cambiar la configuración general.</Alert>}
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={2}>
          <Box>
            ...
            <Typography sx={{ fontSize: '0.875rem', color: '#475569' }}>
              {form?.agendaActiva
                ? 'Se pueden reservar citas desde el sitio, el directorio y WhatsApp.'
                : 'Agenda pausada. Actívala cuando tengas tipos y horario configurados.'}
            </Typography>
          </Box>
          <FormControlLabel
            control={<Switch checked={!!form?.agendaActiva} disabled={!isAdmin}
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} md={6}>
          <TextField fullWidth size="small" type="number" label="Duración por defecto (min)" disabled={!isAdmin}
        ...
        <Grid item xs={12} md={6}>
          <TextField fullWidth size="small" type="number" label="Buffer entre citas (min)" disabled={!isAdmin}
        ...
        <Grid item xs={12} md={6}>
          <TextField fullWidth size="small" type="number" label="Ventana de reserva (días)" disabled={!isAdmin}
            value={form?.bookingWindowDays ?? 60}
            onChange={(e) => upd('bookingWindowDays', parseInt(e.target.value) || 0)}
            helperText="Anticipación máxima a la que se puede reservar." />
        </Grid>
        <Grid item xs={12} md={6}>
          <TextField fullWidth size="small" type="number" label="Anticipación mínima (horas)" disabled={!isAdmin}
        ...
        <Grid item xs={12} md={6}>
          <TextField fullWidth select size="small" label="Zona horaria" disabled={!isAdmin}
        ...
        <Grid item xs={12} md={6}>
          <FormControlLabel
            control={<Switch checked={!!form?.autoConfirm} disabled={!isAdmin}
              onChange={(e) => upd('autoConfirm', e.target.checked)} />}
            label={form?.autoConfirm ? 'Confirmación automática' : 'Requiere aprobación'} />
        </Grid>
      </Grid>
...
// ── Tipos de consulta ──
function TypesTab({ types, setTypes, showSnack, isAdmin }) {
  ...
  const save = async () => {
    ...
    const r = editing.id
      ? await api.patch(`${BASE}/types/${editing.id}`, body)
      : await api.post(`${BASE}/types`, body);
    if (r.error) return showSnack(r.error, 'error');
    setOpen(false);
    const fresh = await api.get(`${BASE}/types?includeInactive=1`);
  ...
  const remove = async (t) => {
    ...
    const r = await api.delete(`${BASE}/types/${t.id}`);
    if (r.error) return showSnack(r.error, 'error');
    ...
    <Box>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
        <Typography sx={{ color: '#475569', fontSize: '0.875rem' }}>
          Los servicios que ofrece OírConecta. Cada uno tiene su propia duración y precio.
        </Typography>
        {isAdmin && (
      ...
      {types.length === 0 ? (
        <EmptyState text="Aún no hay tipos de consulta. Crea al menos uno para poder reservar." />
      ) : (
        <Table size="small">
          <TableHead>
            <TableRow>
...
// ── Horario semanal ──
function AvailabilityTab({ availability, setAvailability, showSnack, isAdmin }) {
  ...
  const addRow = (dow) => setRows((r) => {
    ...
    if (existing.length > 0) {
      ...
      const newStart = Math.min(lastEnd + 120, 22 * 60);
      start = toHHMM(newStart);
      end = toHHMM(Math.min(newStart + 180, 23 * 60));
    }
  ...
  const save = async () => {
    ...
    const r = await api.put(`${BASE}/availability/weekly`, { rows: payload });
    setSaving(false);
    if (r.error) return showSnack(r.error, 'error');
    const fresh = await api.get(`${BASE}/availability`);
  ...
  return (
    <Box>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
        <Box>
          <Typography sx={{ color: '#475569', fontSize: '0.875rem' }}>
            Franjas de atención por día. Puedes tener <strong>varias franjas por día</strong> (ej. mañana 07:00–12:00, tarde 14:00–17:00).
          </Typography>
          <Typography sx={{ color: '#94a3b8', fontSize: '0.75rem', mt: 0.25 }}>
            Tip: para días con hueco de almuerzo usa "Mañana + tarde" — agrega ambas franjas de un click.
          </Typography>
        </Box>
        {isAdmin && (
          <Button variant="contained" startIcon={<SaveOutlined />} onClick={save} disabled={saving}
      ...
      {!isAdmin && <Alert severity="info" sx={{ mb: 2, borderRadius: '10px' }}>Solo un administrador puede modificar el horario semanal.</Alert>}
        ...
        return (
          <Card key={dow} sx={{ mb: 2, border: '1px solid #e5e7eb', borderRadius: '10px' }}>
            <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
              ...
              {dayRows.length === 0 ? (
                <Typography sx={{ fontSize: '0.8125rem', color: '#94a3b8', pl: 0.5 }}>Día sin atención.</Typography>
              ) : (
                <Stack spacing={1}>
                  {dayRows.map(({ r, i }) => (
                    <Stack key={r.id || r._tmp || i} direction="row" spacing={1} alignItems="center">
                      <TextField size="small" type="time" value={r.startTime} disabled={!isAdmin}
                        onChange={(e) => updRow(i, 'startTime', e.target.value)} sx={{ width: 130 }} />
                      <Typography sx={{ color: '#94a3b8' }}>a</Typography>
                      <TextField size="small" type="time" value={r.endTime} disabled={!isAdmin}
                        onChange={(e) => updRow(i, 'endTime', e.target.value)} sx={{ width: 130 }} />
                      <FormControlLabel control={<Switch checked={r.active !== false} size="small" disabled={!isAdmin}
                        onChange={(e) => updRow(i, 'active', e.target.checked)} />}
                        label={r.active !== false ? 'Activa' : 'Pausada'} />
                      {isAdmin && <IconButton size="small" onClick={() => delRow(i)}><DeleteOutlineOutlined fontSize="small" /></IconButton>}
                    </Stack>
                  ))}
                </Stack>
              )}
...
function BlocksTab({ blocks, setBlocks, showSnack }) {
  ...
  const newBlock = () => {
    ...
    setEditing({ startAt: toLocalInput(start), endAt: toLocalInput(end), allDay: false, tipo: '', motivo: '' });
    setOpen(true);
  };
  ...
  const save = async () => {
    ...
    const r = editing.id
      ? await api.patch(`${BASE}/blocks/${editing.id}`, body)
      : await api.post(`${BASE}/blocks`, body);
    if (r.error) return showSnack(r.error, 'error');
    setOpen(false);
    const fresh = await api.get(`${BASE}/blocks`);
  ...
  const remove = async (b) => {
    ...
    const r = await api.delete(`${BASE}/blocks/${b.id}`);
    if (r.error) return showSnack(r.error, 'error');
  ...
  return (
    <Box>
      ...
      {blocks.length === 0 ? (
        <EmptyState text="No hay bloqueos. Se puede reservar en cualquier franja del horario semanal." icon={BlockOutlined} />
      ) : (
        <Table size="small">
          <TableHead>
            <TableRow>
...
function AppointmentsTab({ appointments, setAppointments, showSnack }) {
  ...
  const reload = async () => {
    const r = await api.get(`${BASE}/appointments`);
  ...
  return (
    <Box>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2, flexWrap: 'wrap', gap: 1 }}>
        <Typography sx={{ color: '#475569', fontSize: '0.875rem' }}>
          Citas del centro OírConecta reservadas desde cualquier canal (sitio, directorio, WhatsApp, CRM).
        </Typography>
        <Stack direction="row" spacing={0.5}>
          {[{ v: 'UPCOMING', l: 'Próximas' }, { v: 'TODAY', l: 'Hoy' }, { v: 'ALL', l: 'Todas' }].map((opt) => (
      ...
      {filtered.length === 0 ? (
        <EmptyState text={filter === 'UPCOMING'
          ? 'No hay citas próximas. Aparecerán acá cuando alguien reserve.'
          : filter === 'TODAY' ? 'No hay citas para hoy.'
                    ...
                    {!isCancel && !isPast && (
                      <Tooltip title="Cancelar cita">
                        <IconButton size="small" onClick={async () => {
                          ...
                          const r = await api.patch(`${BASE}/appointments/${a.id}`, { estado: 'CANCELLED' });
                          if (r.error) return showSnack(r.error, 'error');
