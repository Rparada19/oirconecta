...
const TEST = [
  ...
  { key: 'presupuesto', label: 'Presupuesto', opts: ['Económico', 'Medio', 'Alto', 'Sin límite'] },
...
export default function ComparadorPage() {
  ...
  const puedeComparar = completos.length >= 1 && !loading;
  ...
  const enviarLead = async () => {
    ...
    try {
      const res = await fetch(`${getApiBaseUrl()}/api/comparador/leads`, {
        ...
        body: JSON.stringify({
          ...lead,
          marcaSugerida: result?.recomendacion?.ganadorEtiqueta || null,
        ...
        <Container maxWidth="lg" sx={{ py: 8, flex: 1 }}>
          ...
          {loadingItems ? (
            ...
            <>
              ...
              {/* Test */}
              <Typography variant="h6" sx={{ fontWeight: 700, mb: 2 }}>Cuéntanos un poco sobre ti</Typography>
              <Grid container spacing={2} sx={{ mb: 4 }}>
                {TEST.map((q) => (
              ...
              <Box sx={{ textAlign: 'center', mb: 5 }}>
                <Button variant="contained" size="large" startIcon={<AutoAwesomeIcon />} disabled={!puedeComparar} onClick={comparar}
                ...
                {completos.length === 0 && <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1 }}>Completa al menos una opción.</Typography>}
              </Box>
