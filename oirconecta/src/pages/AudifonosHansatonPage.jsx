...
const BRAND = {
  nombre: 'Hansaton',
  logo: '/logos/marcas/Hansaton-logo.png.webp',
  eslogan: 'Acústica Sonova',
  descripcion: 'Audífonos con tecnología de procesamiento de sonido 3D, recargables y conectividad total para una experiencia auditiva personalizada. Jam XT, Beat XT y app Stream Remote para cada necesidad auditiva.',
  rating: '4.7',
...
const productos = [
  { nombre: 'Hansaton Jam XT', categoria: 'RIC', descripcion: 'Audífono con tecnología de procesamiento de sonido 3D, recargable y control total desde la app.', caracteristicas: ['Tecnología de procesamiento 3D', 'Bluetooth y recargable', 'App Stream Remote', 'Resistente al agua'], destacado: true },
  { nombre: 'Hansaton Beat XT', categoria: 'BTE', descripcion: 'Sonido potente y claro con diseño robusto y elegante.', caracteristicas: ['Sonido potente y claro', 'Recargable', 'App Stream Remote', 'Diseño robusto y elegante'], destacado: false },
  { nombre: 'Hansaton Flow XT', categoria: 'ITE', descripcion: 'Diseño personalizado, conectividad avanzada y sonido natural discreto.', caracteristicas: ['Diseño personalizado', 'Conectividad avanzada', 'App Stream Remote', 'Sonido natural y discreto'], destacado: false },
];

const tecnologias = [
  { icon: Hearing, titulo: 'Sonido 3D', descripcion: 'Tecnología de procesamiento de sonido 3D para mejor audición', gradient: 'linear-gradient(135deg, #003DA5 0%, #272F50 100%)' },
  { icon: Bluetooth, titulo: 'Conectividad Total', descripcion: 'Bluetooth y streaming directo a cualquier dispositivo', gradient: 'linear-gradient(135deg, #003DA5 0%, #272F50 100%)' },
  ...
  { icon: Smartphone, titulo: 'App Stream Remote', descripcion: 'Control total de los audífonos desde tu móvil', gradient: 'linear-gradient(135deg, #003DA5 0%, #272F50 100%)' },
];
...
const AudifonosHansatonPage = () => {
  ...
  return (
    <>
      <Helmet>
        <title>Audífonos Hansaton - OírConecta | Acústica Sonova</title>
        <meta name="description" content="Descubre los audífonos Hansaton con tecnología de procesamiento de sonido 3D. Jam XT, Beat XT y Flow XT con conectividad Bluetooth y app Stream Remote." />
        <link rel="canonical" href="https://oirconecta.com/audifonos/hansaton" />
      </Helmet>
      <Header />
      ...
      <Box sx={{
        ...
        background:
          ...
          'linear-gradient(160deg, #00215a 0%, #00215a 30%, #272F50 70%, #1a1f38 100%)',
            ...
            <Grid item xs={12} md={6}>
              ...
              <Box sx={{ display: 'flex', gap: 1, mb: 3, flexWrap: 'wrap' }}>
                ...
                <Chip label="Audífonos" size="small" onClick={() => navigate('/audifonos')}
                  sx={{ bgcolor: 'rgba(255,255,255,0.10)', color: 'rgba(255,255,255,0.75)', border: '1px solid rgba(255,255,255,0.18)', cursor: 'pointer', fontWeight: 600 }} />
                <Chip label="Hansaton" size="small"
              ...
              <Box sx={{ display: 'inline-flex', alignItems: 'center', px: 2.5, py: 1.5,
                ...
                <img src={BRAND.logo} alt="Hansaton" style={{ height: 48, objectFit: 'contain' }} />
              </Box>
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                <Button variant="contained" size="large" endIcon={<ArrowForward />}
                  onClick={() => navigate('/contacto?asunto=Solicitud%20de%20información%20-%20Hansaton')}
                  sx={{ borderRadius: '14px', fontWeight: 800, px: 3.5, py: 1.75, fontSize: '1rem',
                    bgcolor: '#fff', color: '#003DA5',
              ...
              <Box sx={{ display: 'flex', justifyContent: 'center' }}>
                <Box sx={{ borderRadius: '8px', overflow: 'hidden', width: '100%', maxWidth: 440,
                  ...
                  <Box sx={{ borderRadius: '8px', bgcolor: '#fff', p: 3, mb: 3, textAlign: 'center' }}>
                    <img src={BRAND.logo} alt="Hansaton" style={{ height: 80, objectFit: 'contain' }} />
                  </Box>
                  {/* Stats */}
            ...
            {productos.map((p, i) => (
              <Grid item xs={12} sm={6} md={4} key={i}>
                <Box sx={{ height: '100%', borderRadius: '22px', overflow: 'hidden', bgcolor: '#fff',
                  boxShadow: '0 2px 20px rgba(0,61,165,0.08)', border: '1px solid rgba(0,61,165,0.08)',
                  display: 'flex', flexDirection: 'column',
                  transition: 'all 0.28s ease',
                  '&:hover': { transform: 'translateY(-6px)', boxShadow: '0 20px 48px rgba(0,61,165,0.15)' },
                  position: 'relative',
                }}>
                  {p.destacado && (
                    <Box sx={{ position: 'absolute', top: 12, right: 12, zIndex: 2,
                      px: 1.5, py: 0.5, borderRadius: '8px',
                      background: 'linear-gradient(135deg,#003DA5,#272F50)',
                      boxShadow: '0 4px 12px rgba(0,61,165,0.35)' }}>
                  ...
                  <Box sx={{ p: 2.5, flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
                      <Chip label={p.categoria} size="small" sx={{ fontWeight: 700, fontSize: '0.7rem',
                        bgcolor: 'rgba(0,61,165,0.10)', color: '#003DA5', border: '1px solid rgba(0,61,165,0.20)' }} />
                    </Box>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75, mb: 2.5 }}>
                      {p.caracteristicas.slice(0, 3).map((c) => (
                        <Box key={c} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <CheckCircle sx={{ fontSize: 15, color: '#003DA5' }} />
                          <Typography sx={{ fontSize: '0.8125rem', color: '#4a5568' }}>{c}</Typography>
                        </Box>
                      ))}
                    </Box>
                    <Button variant="contained" fullWidth onClick={() => navigate('/contacto?asunto=Solicitud%20de%20información%20-%20Hansaton')}
                      sx={{ borderRadius: '12px', fontWeight: 700,
                        background: 'linear-gradient(135deg,#003DA5,#272F50)',
                        boxShadow: '0 4px 14px rgba(0,61,165,0.25)',
                        '&:hover': { boxShadow: '0 6px 20px rgba(0,61,165,0.35)' } }}>
                      Solicitar información
                    </Button>
                  </Box>
                </Box>
              ...
              return (
                <Grid item xs={12} sm={6} md={3} key={t.titulo}>
                  <Box sx={{ borderRadius: '22px', p: 3, height: '100%', textAlign: 'center',
                    ...
                    border: '1px solid rgba(0,61,165,0.08)', boxShadow: '0 2px 16px rgba(0,61,165,0.07)',
                    transition: 'all 0.28s ease',
                    '&:hover': { transform: 'translateY(-6px)', boxShadow: '0 20px 48px rgba(0,61,165,0.14)' } }}>
                    <Box sx={{ width: 64, height: 64, borderRadius: '8px', background: t.gradient,
                      display: 'flex', alignItems: 'center', justifyContent: 'center', mx: 'auto', mb: 2,
                      boxShadow: '0 8px 20px rgba(0,61,165,0.25)' }}>
      ...
      <Box sx={{ position: 'relative', overflow: 'hidden', py: { xs: 5, md: 8 },
        background:
          'radial-gradient(ellipse 80% 60% at 20% 50%, rgba(0,61,165,0.40) 0%, transparent 55%),' +
        ...
        <Container maxWidth="md" sx={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
          ...
          <Typography sx={{ fontSize: '1.125rem', color: 'rgba(255,255,255,0.78)', mb: 5, maxWidth: 500, mx: 'auto', lineHeight: 1.65 }}>
            Un especialista de la red te orienta sobre los modelos Hansaton que mejor se adaptan a tu caso.
          </Typography>
...
export default AudifonosHansatonPage;
