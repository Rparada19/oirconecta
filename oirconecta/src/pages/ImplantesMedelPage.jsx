  ...
  return (
    <>
            ...
            {productos.map((p, i) => (
              <Grid item xs={12} sm={6} md={3} key={i}>
                <Box sx={{ height: '100%', borderRadius: '22px', overflow: 'hidden', bgcolor: '#fff',
                  boxShadow: '0 2px 20px rgba(201,52,43,0.08)', border: '1px solid rgba(201,52,43,0.08)',
                  display: 'flex', flexDirection: 'column',
                  transition: 'all 0.28s ease',
                  '&:hover': { transform: 'translateY(-6px)', boxShadow: '0 20px 48px rgba(201,52,43,0.15)' },
                  position: 'relative',
                }}>
                  {p.destacado && (
                    <Box sx={{ position: 'absolute', top: 12, right: 12, zIndex: 2,
                      ...
                      boxShadow: '0 4px 12px rgba(201,52,43,0.35)' }}>
                  ...
                  <Box sx={{ p: 2.5, flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
                      <Chip label={p.categoria} size="small" sx={{ fontWeight: 700, fontSize: '0.7rem',
                        bgcolor: 'rgba(201,52,43,0.08)', color: '#C9342B', border: '1px solid rgba(201,52,43,0.18)' }} />
                    </Box>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75, mb: 2.5 }}>
                      {p.caracteristicas.slice(0, 3).map((c) => (
                        <Box key={c} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <CheckCircle sx={{ fontSize: 15, color: '#C9342B' }} />
                    ...
                    <Button variant="contained" fullWidth onClick={() => navigate('/agendar')}
                      sx={{ borderRadius: '12px', fontWeight: 700,
                        ...
                        '&:hover': { boxShadow: '0 6px 20px rgba(201,52,43,0.35)' } }}>
              ...
              return (
                <Grid item xs={12} sm={6} md={3} key={t.titulo}>
                  <Box sx={{ borderRadius: '22px', p: 3, height: '100%', textAlign: 'center',
                    ...
                    border: '1px solid rgba(201,52,43,0.08)', boxShadow: '0 2px 16px rgba(201,52,43,0.07)',
                    transition: 'all 0.28s ease',
                    '&:hover': { transform: 'translateY(-6px)', boxShadow: '0 20px 48px rgba(201,52,43,0.14)' } }}>
                    <Box sx={{ width: 64, height: 64, borderRadius: '8px', background: t.gradient,
                      display: 'flex', alignItems: 'center', justifyContent: 'center', mx: 'auto', mb: 2,
                      boxShadow: '0 8px 20px rgba(201,52,43,0.25)' }}>
      ...
      <Box sx={{ position: 'relative', overflow: 'hidden', py: { xs: 5, md: 8 },
        background:
          'radial-gradient(ellipse 80% 60% at 20% 50%, rgba(201,52,43,0.60) 0%, transparent 55%),' +
