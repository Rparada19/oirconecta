...
async function ensureIaAgentConfigSchema(prisma) {
  try {
    ...
    // Nodos nuevos: conocimiento que guía sin divulgarse, manejo de objeciones
    // y política de precios. Antes no había dónde poner "esto lo sabes pero no
    // lo dices", y terminaba mal metido en avoidTopics o en technologies.
    await prisma.$executeRawUnsafe(`ALTER TABLE "ia_agent_config" ADD COLUMN IF NOT EXISTS "internalKnowledge" TEXT`);
    await prisma.$executeRawUnsafe(`ALTER TABLE "ia_agent_config" ADD COLUMN IF NOT EXISTS "objections" TEXT`);
    await prisma.$executeRawUnsafe(`ALTER TABLE "ia_agent_config" ADD COLUMN IF NOT EXISTS "pricing" TEXT`);
    ...
    if (!hay.has('EMAIL')) {
      await prisma.notificationTemplate.create({
        data: {
          ...
          subject: 'Gracias por confiarnos tu audición, {{nombre}}',
          body: [
            'Hola {{nombre}},',
            '',
            'Hoy fue un gusto acompañarte.',
            '',
            'Para nosotros una cita nunca es solo una cita. Detrás de cada consulta hay',
            'algo mucho más grande: volver a escuchar la voz de un nieto, entender lo que',
            'se dice en la mesa sin tener que adivinar, dejar de sentirse afuera de la',
            'conversación. Eso es lo que nos mueve todos los días.',
            '',
            'Gracias por confiarnos algo tan importante. No lo damos por sentado.',
            '',
            'Si hoy te sentiste bien atendido, contarlo en unas líneas nos alegra el día',
            'y le da luz a otras personas que llevan años dudando si dar el paso:',
            '{{link_google}}',
            '',
            'Y si algo no salió como esperabas, queremos saberlo primero nosotros.',
            'Respóndenos por aquí y lo resolvemos.',
            '',
            'Con cariño,',
            'Equipo OírConecta',
          ].join('\n'),
    ...
    if (!hay.has('WHATSAPP')) {
      await prisma.notificationTemplate.create({
        data: {
          ...
          body: 'Hola {{nombre}}, hoy fue un gusto acompañarte. Para nosotros una cita nunca es solo una cita: detrás de cada consulta hay alguien que quiere volver a escuchar a los suyos. Gracias por confiarnos algo tan importante. Si te sentiste bien atendido, cuéntalo aquí: {{link_google}} — leer esas palabras nos alegra el día.',
...
async function ensureAppointmentCancellationColumns(prisma) {
  try {
    ...
    await prisma.$executeRawUnsafe(`ALTER TABLE "whatsapp_conversations" ADD COLUMN IF NOT EXISTS "recuperadoAt" TIMESTAMP`);
