...
const getAll = async (req, res, next) => {
  try {
    const { fecha, estado, patientEmail, patientId, page = 1, limit = 50 } = req.query;
    const result = await appointmentsService.getAll({ fecha, estado, patientEmail, patientId, page: parseInt(page), limit: parseInt(limit) });
